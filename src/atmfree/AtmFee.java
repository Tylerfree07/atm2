...
public class AtmFee {
    ...
    private Map<String, List<Transaction>> transactions = new HashMap<>();
     ...
     private String API_KEY = "3d3ad73d8e9fda40a5af4915";
    private String BASE_URL = "https://v6.exchangerate-api.com/v6/" + API_KEY + "/latest/USD";
     private final String fromEmail =  "dashergaming07@gmail.com";
...
while (true) {
    ...
    switch (choice) {
                case 1 -> transactions();
                case 2 -> exportTransactionsToPDF();
                case 3 -> exportTransactionsToCSV();
                case 4 ->  {clearTransactionsHistory(accountNumber);}
                case 5 -> { return; }
                default -> System.out.println("Invalid option. Try again.");
            }
        }
    ...
    private void clearTransactionsHistory(String accountNumber) {
        System.out.println("------------------------"); 
    ...
    private void viewBalence() {
        ...
        convertedBalance = Math.round(convertedBalance * 100) / 100.0;
        System.out.println("------------------------");
